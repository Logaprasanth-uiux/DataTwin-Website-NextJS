...
| **Home hero prompt flow + `/get-estimate`** | *(new — no static source)* | ✅ first pass — **front-end mock**. The homepage hero composer now responds: on submit it shows a processing checklist then matches the visitor's problem to 1–3 of the 11 use cases (`components/homeMatch.ts` — keyword scoring + drafted "what we'd test" blurbs, team edits in place), each linking to its product page. "Get an estimate" stashes `{problem, matches}` in `sessionStorage['dt-estimate-context']` and full-loads `/get-estimate` — a **third root layout group** `app/(estimate)/` (own `<html>`, slim `.est-header`, shared `globals.css` + fonts + `dt-theme`) that opens straight on the "where do we send the estimate?" contact form (no AI-agent intro; a plain `#estIntro` line shows instead when there's no stashed problem). The composer answers questions from a mock knowledge base (`components/estimateAnswers.ts` — keyword match over ~14 hand-written answers drawn from the site copy + a fallback; team refines / dev replaces with a real assistant). Form + answers are mocked (no network). The stashed context is **one-shot** — read once on load then cleared, and ignored if older than 15 min, plus a `pageshow`/bfcache reload guard — so every CTA and every reload opens `/get-estimate` fresh; only the hero "Get an estimate" carries the just-stated problem. New CSS: `.hero-thread/.hero-proc/.hero-match*`, `.est-*`, `.est-typing`, `.recov-cta` in `app/globals.css`. The homepage "problem" CTAs + site-chrome CTAs now route here: page.tsx "Tell us your situation / Tell us yours / Tell us your problem" + a new "Get an estimate" button under the `#recoverable` cards, header "Get an estimate" · "Contact" · "Recovery Estimator", footer "Contact" · "Tell us your problem", drawer "Contact" · "Recovery Estimator". Left as-is: "What can we recover?" (→ `#recoverable`) and the ~10 product-page `Get an estimate for X → /#cta` CTAs. |
...
components/
  ...
  estimateAnswers.ts plain .ts — mock Q&A knowledge base for the /get-estimate composer (demo only)
  EstimateChatScripts.tsx   'use client' — /get-estimate: recap hydrate, form validate/submit mock, composer answers (mock)
